...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("pipeline_execution_Random_player_f_classifier_simulation.log", mode='a'), # Appends to file
...
def get_total_tagged_data_so_far(sub, llm, max_week_index, target_subsample):
    ...
    """
    directory_path = f'bootstrap_results_Random_player_f_classifier'
...
def genAI_turn(weekly_data, sample_index):
    if len(weekly_data) > MAXIMUM_NUMBER_OF_QUESTIONS_PER_WEEK_BY_GENAI:
        weekly_data = weekly_data.sample(
            n=MAXIMUM_NUMBER_OF_QUESTIONS_PER_WEEK_BY_GENAI,
...
def save_weekly_results(weekly_data, week_index, subsample, sub, llm):
    output_directory = f'bootstrap_results_Random_player_f_classifier'
    ...
    for sub in SUBJECTS:
        ...
        for week_index, week in tqdm(enumerate(weeks), desc=f"Processing weeks for {llm}"):
            if ((week_index+1) % 13 == 0 and week_index != 51) or week_index==0: 
                path=f"../partial_collaboration_Bootstrap/Player-F-Models/united_model_{week_index//13}"
                forum = Forum_Entity(model_name=path, num_labels=2)
                forum.threshold = Thresholds.get(f"united_model_{week_index//13}", 0.83)
            ...
            for i in range(50):
                ...
                weekly_data = genAI_turn(weekly_data, i+1)
            ...
            # Fetching summary for the end of the week using the last sample's context (i+1)
            total_results_df = get_total_tagged_data_so_far(sub, llm, week_index+1, i+1) 
            if not total_results_df.empty:
                total_results = total_results_df[(total_results_df['genai_label'] == True) & (total_results_df['AnswerCount'].gt(0))][GEN_AI_UTILITY].sum()
                logger.info(f"--- WEEK {week_index} SUMMARY [{llm}] ---")
